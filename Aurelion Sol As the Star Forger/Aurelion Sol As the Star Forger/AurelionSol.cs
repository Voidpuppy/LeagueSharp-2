...
namespace Aurelion_Sol_As_the_Star_Forger {
	internal class AurelionSol {
		...
		private static void Spellbook_OnCastSpell(Spellbook sender, SpellbookCastSpellEventArgs args) {
			...
			{
				Game.SendPing(PingCategory.OnMyWay,args.EndPosition);
			}
		}
		...
		private static void Drawing_OnDraw(EventArgs args) {
			...
			{
				var eRange = new[] { 3000, 4000, 5000, 6000, 7000 }[E.Level - 1];
				Utility.DrawCircle(Player.Position, eRange, EMiniShow.Color,2,25,true);
			}
		...
		private static double GetDmg(Obj_AI_Base target,double amount) {
			...
			if (target.Type == GameObjectType.obj_AI_Hero && (target as Obj_AI_Hero).ChampionName == "Blitzcrank" && !target.HasBuff("BlitzcrankManaBarrierCD") && !target.HasBuff("ManaBarrier"))
			{
		...
		private static void LoadMenu() {
			...
			Game.PrintChat("Aurelion Sol".ToHtml(32) + "  迎接星辰之力吧，凡人 !".ToHtml("#D15FEE", FontStlye.Bold));

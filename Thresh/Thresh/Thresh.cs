...
namespace Thresh {
	class Thresh {
		...
		public static List<Obj_AI_Hero> Qignored = new List<Obj_AI_Hero>();
		...
		private static Obj_AI_Hero GetFurthestAlly() {
		...
		private static Obj_AI_Hero GetTarget() {
			...
			{
				if (adc.Target != null && adc.Target is Obj_AI_Hero && adc.Target.IsValid && Config.Item("QList" + adc.Target.NetworkId).GetValue<bool>())
				{
		...
		private static void TestQDraw() {
			string text = string.Format("Q名字：{0}\n是否是蓄力技能{1}\nToggleState{2}\nIsSealed{3}",
				Q.Instance.Name,
				Q.IsChargedSpell,
				Q.Instance.ToggleState,
				Q.Instance.IsSealed);
			font.DrawText(null, text,
				(int)Drawing.WorldToScreen(Player.Position).X - 50,
				(int)Drawing.WorldToScreen(Player.Position).Y,
				 new ColorBGRA(Color.GreenYellow.B, Color.GreenYellow.G, Color.GreenYellow.R, Color.GreenYellow.A));
		}
	}
}

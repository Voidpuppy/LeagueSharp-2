namespace Valvrave_Sharp
{
    ...
    internal class Program
    {
        ...
        private static void OnLoad(object sender, EventArgs e)
        {
            ...
            {
                Game.PrintChat(string.Format("VS合集 => {0} 不支持!", Player.ChampionName));
                return;
            }
            ...
            MainMenu = new Menu("ValvraveSharp", "VS合集", true, Player.ChampionName).Attach();
            Config.Separator(MainMenu, "Author", "作者: Brian");
            Config.Separator(MainMenu, "Paypal", "贝宝: dcbrian01@gmail.com");
        ...
        private static void UpdateCheck()
        {
            try
            {
                using (var web = new WebClient())
                {
                    ...
                    {
                        Game.PrintChat(string.Format("VS合集 => 当前版本已过时，最新版本: {0}", gitVersion));
                    }
                }
            }
            catch (Exception ex)
